...
import de.gematik.zeta.zetaguard.keycloak.plugins.toJWKS
import de.gematik.zeta.zetaguard.keycloak.plugins.toPublicKey
...
open class ZetaGuardTokenExchangeProvider(private val keystoreService: KeystoreService, private val opaConfig: OPAConfig) :
  ...
  //   https://ey-fp-dev.atlassian.net/browse/ZETAP-793
  //   TODO: Delete me when client is ready
  private val clientAttestationCheckEnable: Boolean by lazy {
    val enable = System.getenv("CHECK_CLIENT_ATTESTATION_ENABLED") ?: "false"
   ...
   * point.
   */
  override fun tokenExchange() =
    verifyX5CCertificate(ZetaGuardTokenExchangeContext(this))
  ...
  private fun handleClientStatementData(context: ZetaGuardTokenExchangeContext): KeycloakValidation = either {
    if (clientAttestationCheckEnable) { // TODO: Delete me when client is ready
      val claims = clientAssertionClaims(context)
      ...
      when (clientStatementData.postureType) {
        ...
        // TODO
        else -> logger.warn("Client attestation check not yet implemented for ${clientStatementData.postureType} posture type")
      }
  ...
  private fun checkClientPublicKeyConsistency(context: ZetaGuardTokenExchangeContext): KeycloakValidation = either {
    ...
    val clientPublicKey = publicKeyString.toPublicKey()
    val jwks = jwksString.toJWKS().bind().also { ensure(it.keys.isNotEmpty()) { invalidClientClaim("Invalid client JWKS: no keys found") } }
    val jwk = jwks.keys[0]
    val clientKeyWrapper = getClientPublicKeyWrapper()
    val jwkPublicKey = jwk.toPublicKey().bind()
